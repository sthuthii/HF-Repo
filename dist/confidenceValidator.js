...
exports.ConfidenceValidator = void 0;
class ConfidenceValidator {
    /**
     * Compute honest health report based on real data.
     */
    static computeHealth(options) {
        const { mockTestAccuracy = 0.75, fileConfidences = [], suspiciousFlags = 0, } = options;
        ...
        const overallHealth = confidenceScore * 0.4 + testScore * 0.3 + suspiciousScore * 0.3;
        ...
        if (overallHealth >= 0.85) {
            ...
            healthLabel = "WARNING";
            recommendedActions.push("Improve file classification accuracy. Many files have low confidence.");
        }
        else {
            healthLabel = "CRITICAL";
            recommendedActions.push("System is unreliable. Model confidence is too low for production use.");
        }
        // Add specific recommendations
        if (avgConfidence < 0.5) {
            recommendedActions.push(`Real-world confidence is only ${(avgConfidence * 100).toFixed(1)}%. Consider adding more training data or refining patterns.`);
        }
        if (lowConfidence > filesAnalyzed * 0.3) {
            recommendedActions.push(`${lowConfidence}/${filesAnalyzed} files have very low confidence (<40%). Review classification rules.`);
        }
        if (suspiciousFlags > 0) {
            recommendedActions.push(`${suspiciousFlags} suspicious classifications detected. Review flagged files manually.`);
        }
        if (mockTestAccuracy === 1.0 && avgConfidence < 0.6) {
            recommendedActions.push("⚠️  DOGFOODING GAP: Perfect mock tests but poor real-world confidence. Mock tests may be too easy.");
        }
        return {
            mockTestAccuracy,
...
//# sourceMappingURL=confidenceValidator.js.map