...
exports.SAMPLE_FILES = void 0;
exports.main = main;
const index_1 = require("./index");
const productionOutput_1 = require("./productionOutput");
// ============================================
// SAMPLE REPOSITORY
// ============================================
const SAMPLE_FILES = {
...
exports.SAMPLE_FILES = SAMPLE_FILES;
// ============================================
// DEMO FUNCTIONS
// ============================================
function demoFileClassification() {
    console.log("\n" + "=".repeat(60));
    console.log("DEMO 1: FILE CLASSIFICATION");
    console.log("=".repeat(60));
    const classifier = new index_1.FileClassifier();
    const testFiles = [
        "src/components/UserCard.tsx",
        "src/api/userController.ts",
        "src/services/AuthService.ts",
    ...
    for (const filePath of testFiles) {
        ...
        console.log(`\nFile: ${filePath}`);
        console.log(`Primary Role: ${result.primaryRole} (confidence: ${result.confidence.toFixed(2)})`);
        console.log("Scores:");
...
function demoDataFlowTracing() {
    ...
    for (let i = 0; i < Math.min(flows.flows?.length ?? 0, 2); i++) {
        ...
        if (!flow)
            continue;
        console.log(`Flow ${i + 1} (Confidence: ${(flow.confidence * 100).toFixed(0)}%):`);
...
function demoMultiRoleComparison() {
    console.log("\n" + "=".repeat(60));
    console.log("DEMO 4: MULTI-ROLE COMPARISON");
    console.log("=".repeat(60));
    const system = (0, index_1.createSystem)();
    system.initializeRepository(SAMPLE_FILES);
    const roles = [index_1.Role.FRONTEND, index_1.Role.BACKEND];
...
function demoFileDetails() {
    ...
    const role = index_1.Role.BACKEND;
...
function demoRepositoryOverview() {
    console.log("\n" + "=".repeat(60));
    console.log("DEMO 6: REPOSITORY OVERVIEW");
    console.log("=".repeat(60));
    const system = (0, index_1.createSystem)();
    system.initializeRepository(SAMPLE_FILES);
    const rolesToTest = [index_1.Role.BACKEND, index_1.Role.FRONTEND, index_1.Role.DEVOPS];
    for (const role of rolesToTest) {
...
function demoPromptTemplates() {
    console.log("\n" + "=".repeat(60));
    console.log("DEMO 7: LLM PROMPT TEMPLATES");
    console.log("=".repeat(60));
    const fileContent = SAMPLE_FILES["src/api/userController.ts"];
    const prompt1 = index_1.ExplanationEngine.summarizeFileForRole(fileContent, index_1.Role.BACKEND, "src/api/userController.ts");
    console.log("\nPrompt Template 1: Summarize for Backend");
    console.log("-".repeat(40));
    console.log(prompt1);
    console.log("\n[Example LLM Response]:");
    console.log("This controller handles user CRUD operations with two main methods: getUserById");
    console.log("retrieves a user by ID from the database and returns a 404 if not found, while");
    console.log("createUser accepts email and password in the request body to create a new user");
    console.log("record. Both methods return JSON responses with success status and data payload.");
    const prompt2 = index_1.ExplanationEngine.explainDataFlow(["UserCard.tsx", "api/index.ts", "userController.ts", "User.ts"], "UserCard.tsx", "User.ts");
    console.log("\nPrompt Template 2: Explain Data Flow");
    console.log("-".repeat(40));
    console.log(prompt2);
    console.log("\n[Example LLM Response]:");
    console.log("UserCard renders user data by calling useUser hook, which calls the API endpoint");
    console.log("in api/index.ts, forwarding the request to userController.ts that queries the User");
    console.log("model to fetch data from the database, then returns the user object through the");
...
function main() {
    console.log("\n");
    console.log("=".repeat(60));
    console.log(" ROLE-AWARE INTELLIGENT REPOSITORY VIEWS DEMO (TypeScript) ".padEnd(60));
    console.log("=".repeat(60));
...
function demoProductionOutput() {
    ...
    try {
        ...
        const output = productionOutput_1.ProductionOutput.transform({
