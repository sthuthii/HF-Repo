...
class ExplanationEngine {
     ...
     */
    static explainFileRelevance(filePath, role, score, primaryRole, reason = "") {
        ...
        if (score > 0.7) {
            ...
            else if (filePath.toLowerCase().includes("component") ||
            ...
            else if (filePath.toLowerCase().includes("util") ||
            ...
            else if (filePath.toLowerCase().includes("api") &&
                role === config_1.Role.FRONTEND) {
                return `Backend API - frontend calls this to fetch/update data.`;
            }
            else if (filePath.toLowerCase().includes("component") &&
                role === config_1.Role.BACKEND) {
     ...
     */
    static getRoleFocus(role) {
        const focusMap = {
            frontend: "UI components, state management, styling, and user interactions",
     ...
     */
    static summarizeFileForRole(fileContent, role, filePath = "") {
...
Summary for ${role}:`;
     ...
     */
    static explainDataFlow(path, sourceFile, targetFile) {
