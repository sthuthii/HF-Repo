...
exports.ProductionOutput = void 0;
exports.demonstrateProductionOutput = demonstrateProductionOutput;
const dependencyParser_1 = require("./dependencyParser");
const suspiciousClassifier_1 = require("./suspiciousClassifier");
const tieBreaker_1 = require("./tieBreaker");
const confidenceValidator_1 = require("./confidenceValidator");
const outputFormatter_1 = require("./outputFormatter");
const repoAnalyzer_1 = require("./repoAnalyzer");
const repositorySummary_1 = require("./repositorySummary");
/**
...
class ProductionOutput {
    ...
    static transform(options) {
        const { classificationResults, fileContents = {}, rawDependencies = {}, dependents = {}, mockTestAccuracy = 0.75, } = options;
        ...
        const tieResolvedResults = classificationResults.map((result) => {
            const tieStatus = tieBreaker_1.TieBreaker.detectTie(result.scores);
            if (tieStatus.isTie) {
                const resolution = tieBreaker_1.TieBreaker.breakTie(result.file, tieStatus.tiedRoles, result.scores);
                return {
                    ...result,
        ...
        for (const [file, rawDep] of Object.entries(rawDependencies)) {
            cleanedDeps[file] = dependencyParser_1.DependencyParser.parseDependencyList(rawDep);
        }
        // STEP 3: Flag suspicious classifications
        console.log("🚨 Step 3: Detecting suspicious classifications...");
        const suspiciousFlags = suspiciousClassifier_1.SuspiciousClassifier.flagMultiple(tieResolvedResults.map((result) => ({
            file: result.file,
            role: result.primaryRole,
            confidence: result.confidence,
            topScores: result.scores,
        })));
        const suspicionSummary = suspiciousClassifier_1.SuspiciousClassifier.summarize(suspiciousFlags);
        console.log(`   Found ${suspicionSummary.total} issues (${suspicionSummary.critical} critical, ${suspicionSummary.warnings} warnings)`);
        ...
        const healthReport = confidenceValidator_1.ConfidenceValidator.computeHealth({
            mockTestAccuracy,
        ...
        // Group by role
        const byRole = outputFormatter_1.OutputFormatter.groupByRole(fileGroups);
        // Build repository output
        const repoOutput = {
            summary: this.buildSummary(tieResolvedResults, suspiciousFlags, healthReport),
            byRole,
            issues: {
                ...
                tied: tieResolvedResults
                    .filter((r) => r.wasTied)
                    .map((r) => ({ file: r.file, roles: r.tiedWith || [] })),
            },
            statistics: {
        ...
        // Format for display
        const formattedOutput = outputFormatter_1.OutputFormatter.formatRepositoryCLI(repoOutput);
        const healthReportStr = confidenceValidator_1.ConfidenceValidator.formatReport(healthReport);
        const issuesReportStr = this.buildIssuesReport(suspiciousFlags, repoOutput.issues.lowConfidence, repoOutput.issues.tied);
        // Generate repository summary
        console.log("📊 Step 6: Generating repository summary...");
        const repoAnalysis = repoAnalyzer_1.RepositoryAnalyzer.analyze({
            classifications: tieResolvedResults,
            dependencies: cleanedDeps,
            dependents,
        });
        const repositorySummary = repositorySummary_1.RepositorySummaryGenerator.generate({
            classifications: tieResolvedResults,
            dependencies: cleanedDeps,
            dependents,
            analysis: repoAnalysis,
        });
        const repositorySummaryStr = repositorySummary_1.RepositorySummaryGenerator.formatForCLI(repositorySummary);
        return {
            formattedOutput,
     ...
     */
    static determinePriority(result, index) {
     ...
     */
    static buildRoleDistribution(results) {
        const dist = {};
     ...
     * Build summary text.
     */
    static buildSummary(results, flags, health) {
        let summary = `\n📋 SUMMARY\n`;
        summary += `${"=".repeat(60)}\n`;
     ...
     * Build issues report.
     */
    static buildIssuesReport(suspicious, lowConf, tied) {
        let report = `\n⚠️  ISSUES REPORT\n`;
        report += `${"=".repeat(60)}\n`;
 ...
 * Simple CLI example.
 */
async function demonstrateProductionOutput() {
    ...
    const mockDeps = {
        ...
        "package-lock.json": 'integrity, "sha512", {, dependencies;',
        "debug_counts.py": "os, sys, \"./utils\", pathlib",
    };
...
//# sourceMappingURL=productionOutput.js.map