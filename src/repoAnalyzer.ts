...
export class RepositoryAnalyzer {
  ...
  static analyze(options: {
    ...
    dependencies: Record<string, string[]>; // file -> list of dependencies
    dependents?: Record<string, string[]>; // file -> list of files that depend on it
  }): RepositoryAnalysis {
    const {
      classifications,
      dependencies,
    ...
    const externalDeps = this.extractExternalDependencies(
      dependencies,
      classifications
    );
    const internalDeps = this.extractInternalDependencies(
      dependencies,
      classifications
    );
  ...
  private static extractExternalDependencies(
    ...
    return Array.from(depMap.values())
      .map((node) => ({
        ...node,
        criticality: (
          node.count >= 10 ? "critical" : 
          node.count >= 5 ? "important" : 
          node.count >= 2 ? "normal" : 
          "low"
        ) as "critical" | "important" | "normal" | "low",
      }))
  ...
  private static identifyPatterns(
    ...
    const patterns: Record<
      string,
      { count: number; roles: Set<string>; description: string }
    > = {};
  ...
  private static findKeyFiles(
    ...
    for (const [file, entry] of keyFiles.entries()) {
      const classification = classifications.find((c) =>
        c.file.includes(file) || file.includes(c.file.split("/").pop() || "")
      );
   ...
   */
  private static getKeyFileReason(inDegree: number, outDegree: number): string {
    if (inDegree >= 10) {
