...
import { RepositoryAnalyzer, RepositoryAnalysis } from "./repoAnalyzer";

export interface RepositorySummary {
  overview: string; // One-line summary
  description: string; // 2-3 sentence description
  architecture: string; // What's the overall structure
  keyTechnologies: string[]; // Main tech stack
  criticalFiles: string[]; // Must-understand files
  architecturalLayers: string[]; // Tiers identified
...
export class RepositorySummaryGenerator {
  ...
  static generate(options: {
    ...
    const repoAnalysis = analysis || RepositoryAnalyzer.analyze({
      classifications,
      dependencies,
      dependents,
    });
    ...
    const architecture = this.determineArchitecture(
      classifications,
      repoAnalysis
    );
   ...
   */
  private static extractKeyTechnologies(analysis: RepositoryAnalysis): string[] {
    const techs: string[] = [];
  ...
  private static generateDescription(
    ...
    const largestRole = Object.entries(stats.roleDistribution).sort(
      ([, a], [, b]) => b - a
    )[0];

    const techString =
      keyTechs.length > 0 ? ` Built with ${keyTechs.slice(0, 3).join(", ")}.` : "";
  ...
  private static generateRecommendations(
    ...
    if (stats.avgConfidence < 0.6) {
      recommendations.push(
        "Low average confidence - review classifier keywords or provide more training data"
      );
    }
    ...
    if (totalFiles > 100) {
      recommendations.push(
        "Large codebase - focus on key files first before deep analysis"
      );
    }
    ...
    if (analysis.keyPatterns.length === 0) {
      recommendations.push(
        "No clear architectural patterns detected - may benefit from refactoring"
      );
    }

    // Dependency recommendations
    const criticalDeps = analysis.externalDependencies.filter(
      (d) => d.criticality === "critical"
    );
    if (criticalDeps.length > 0) {
      recommendations.push(
        `${criticalDeps.length} critical external dependencies - ensure they're well-tested`
      );
    }

    // Role distribution recommendations
    const singleRoleFiles = Object.values(stats.roleDistribution).filter(
      (count) => count === 1
    ).length;
    if (singleRoleFiles > 5) {
      recommendations.push(
        "Many isolated role files - may indicate scattered functionality"
      );
    }
  ...
  private static determineComplexity(
    stats: { totalFiles: number; roleDistribution: Record<string, number> },
    analysis: RepositoryAnalysis,
    classifications: FileClassificationResult[]
